...
```{r plot-date-vs-stepsByDate-noNA}
...
`r sprintf("Steps taken per day: mean=%6.1f, median=%6.1f", meanSteps, medianSteps)`
...
```
```{r time-series-plot-interval-vs-average-steps}
...
The 5-minute interval with the largest average number of steps starts at `r intervalStartHHMM` (in 24-hour military time).
It contains `r trunc(maxStepInterval$steps)` steps.
...
```
The number of step-interval observations (rows) with missing values is `r nIncompleteCases`.
The number of these incomplete rows with missing (NA) steps value is `r nStepsAreNA` 
(`r naStepsVisavisIncompleteCases`).
...
* Number of entries in stepdf_imputed with NA steps: `r sum(is.na(stepdf_imputed$steps))` (should be 0)
...
```
`r  nNaSteps == nNewlyNonNaSteps` (`r nNaSteps` == `r nNewlyNonNaSteps`)
...
```{r plot-date-vs-stepsByDate-NAimputed}
...
```
`r sprintf("Steps, including imputed, taken per day: mean=%6.1f, median=%6.1f", 
    meanSteps_imputed, medianSteps_imputed)`
...
```{r time-series-plot-interval-vs-average-steps-weekend-and-weekday}
## TODO: Improve x-axis labeling.
par(mfrow=c(2,1))
